...
void UInv_InventoryItem::SetTotalStackCount(int32 StackCount)
{
	TotalStackCount = FMath::Max(0, StackCount);
	if (FInv_StackableFragment* StackableFragment = GetItemManifestMutable().GetFragmentOfTypeMutable<FInv_StackableFragment>())
	{
		StackableFragment->SetStackCount(TotalStackCount);
	}
}
